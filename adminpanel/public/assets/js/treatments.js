(() => {
  ...
  source?.addEventListener('input', () => {
    ...
    if (previewName) previewName.textContent = source.value || 'Treatment name';
  });
  form.querySelector('[name="category"]')?.addEventListener('input', (event) => {
    const preview = document.querySelector('[data-preview-category]');
    if (preview) preview.textContent = event.target.value || 'Category';
  });
  form.querySelector('[name="introduction"]')?.addEventListener('input', (event) => {
    const preview = document.querySelector('[data-preview-intro-text]');
    if (preview) preview.textContent = event.target.value || 'Introduction preview will appear here.';
  });
